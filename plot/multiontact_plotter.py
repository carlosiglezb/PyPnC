...
class MulticontactPlotter:
    ...
    def get_full_to_trajectories(self):
        ...
        for it_num in range(len(horizon_lst)):
            ...
            next_cont_idx, next_idx = self.get_next_idxs(b_has_impulse, curr_cont_idx, curr_idx, horizon_lst, it_num)
            ...
            # if self._robot_planner.solver_type == 'single':
            if b_has_impulse:
                curr_idx = sum(horizon_lst[:it_num+1]) + it_num + 1
                # curr_idx += horizon_lst[it_num]     # when not adding impulse model
            else:   #(us) == sum(horizon_lst):
                curr_idx += horizon_lst[it_num]
            curr_cont_idx += horizon_lst[it_num]

        return phase, time, us_l_reduced, us_larm_reduced, us_r_reduced, us_rarm_reduced, xs_l_reduced, xs_larm_reduced, xs_r_reduced, xs_rarm_reduced

    def get_next_idxs(self, b_has_impulse, curr_cont_idx, curr_idx, horizon_lst, it_num):
        if b_has_impulse:
            next_idx = curr_idx + horizon_lst[it_num]  # do not consider index before impact
            next_cont_idx = curr_cont_idx + horizon_lst[it_num]  # next index on continuous variables
            # next_idx = curr_idx + horizon_lst[it_num]   # do not consider index before impact
            # next_cont_idx = curr_cont_idx + horizon_lst[it_num]
        else:
