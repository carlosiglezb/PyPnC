...
class Box:
    ...
    def plot3d(self, ax, color=(1, 0, 0, 0.5)):
        import numpy as np

        assert self.d == 3

        # create 3D cube using voxel size
        voxel_size = 0.1
        x, y, z = np.meshgrid(np.arange(self.l[0], self.u[0], voxel_size),
                              np.arange(self.l[1], self.u[1], voxel_size),
                              np.arange(self.l[2], self.u[2], voxel_size))
        cube = (x >= self.l[0]) & (y >= self.l[1]) & (z >= self.l[2]) & \
               (x <= self.u[0]) & (y <= self.u[1]) & (z <= self.u[2])

        # create [x,y,z] location of voxel
        x_new, y_new, z_new = np.meshgrid(np.arange(self.l[0], self.u[0]+0.09, voxel_size),
                                          np.arange(self.l[1], self.u[1]+voxel_size, voxel_size),
                                          np.arange(self.l[2], self.u[2]+voxel_size, voxel_size))
        ax.voxels(x_new, y_new, z_new, cube, facecolors=color)
        ax.set_xlabel('x')
        ax.set_ylabel('y')
