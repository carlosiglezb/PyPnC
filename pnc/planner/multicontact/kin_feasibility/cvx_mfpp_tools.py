...
def add_vel_acc_constr(f_name, seg_surface_normal, point, constraints, lbg, ubg, b_constr_accel=True):
    ...
    if seg_surface_normal is not None:
        ...
        constraints.append(-normal_tilde.reshape(-1, 1).T @ point[BezierParam.VEL.value][-2, :].T)
        lbg.append(0.)
...
def add_vel_acc_constr_casadi(f_name, seg_surface_normal, point, constraints, lbg, ubg, b_constr_accel=True):
    ...
    if b_constr_accel:
        # apply only strictly positive and negative accelerations
        if surf_normal[Axis.X.value] > 0:
            constraints.append(point[BezierParam.ACC.value][-1,:][Axis.X.value])  # pos acc
            lbg.append(0.)
            ubg.append(casadi.inf)
        elif surf_normal[Axis.X.value] < 0:
            constraints.append(point[BezierParam.ACC.value][-1,:][Axis.X.value])  # neg acc
