...
def get_fully_opposing_limbs_contact_sequence(stairs: TiltedStairs,
                                        starting_pose: dict[str: np.ndarray],
                                        robot_name: str = 'g1',
                                        b_use_knees: bool = True):
    box_width = stairs.box_width
    ...
    if robot_name == 'g1':
        ...
        lh2_height = 1.5
        rh3_height = 1.9
        final_ankle_height = 0.08

        lh1_wall = np.array([0.20, 0.32, lh1_height])
        rh1_wall = np.array([0.26, -0.32, rh1_height])
    else:
        raise ValueError(f"Unknown tilted stair settings for robot: {robot_name}")
    delta_h_left = (box_h2_left - box_h1_left)
    delta_h_right = (box_h2_right - box_h1_right)
    ...
    lh_wall1_contact = PlannerSurfaceContact('LH', lh_wall_normal)
    motion_frames_seq.add_contact_surfaces([lh_wall1_contact])
    ...
    if b_use_knees:
        ...
        motion_frames_seq.add_motion_frame({
            ...
            'RH': rh1_wall})
    lf_step1_contact = PlannerSurfaceContact('LF', left_step_normal)
    rh_step1_contact = PlannerSurfaceContact('RH', rh_wall_normal)
    motion_frames_seq.add_contact_surfaces([lf_step1_contact, rh_step1_contact])
