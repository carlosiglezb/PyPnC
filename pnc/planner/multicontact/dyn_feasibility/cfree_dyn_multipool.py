...
def plan_kin_dyn(planner_args):
    ...
    # return timings of solvers
    return kin_solver_stats, dyn_solver_stats
...
def main(args):
    ...
    kin_args = []
    num_simultaneous_plans = 3
    for i in range(num_simultaneous_plans):
        kin_args.append(kin_args_seq1)
    # Create processes for each planning task using Pools
    with multiprocessing.Pool(processes=3) as pool:
        res = pool.map(plan_kin_dyn, kin_args)

    # Save results to pkl file
    sca_str = '_' + str(num_simultaneous_plans)
    if B_USE_SELF_COLLISION_AVOIDANCE:
        sca_str = '_sca_' + str(num_simultaneous_plans)
    data_saver = DataSaver('RAL/g1_kin_dyn' + sca_str + '_solve_stats.pkl')
    for r in res:
        data_saver.add('kin_solver_stats', r[0])
        data_saver.add('dyn_solver_stats', r[1])
        data_saver.advance()
    data_saver.close()
