...
def get_five_stage_two_hand_contact_sequence(robot_name, door_x_wall_pos: float, final_pose: dict):
    if robot_name == 'g1':
        # G1 settings
        door_l_inner_location = np.array([door_x_wall_pos, 0.35, 1.1])
        door_r_inner_location = np.array([door_x_wall_pos, -0.35, 1.1])
    else:
        # ergoCub settings
        door_l_inner_location = np.array([door_x_wall_pos, 0.35, 1.0])
        door_r_inner_location = np.array([door_x_wall_pos, -0.35, 1.0])
    ...
    # ---- Step 4: square up (both hands release to final position)
    if B_USE_KNEES:
        fixed_frames.append(['torso', 'LF', 'RF', 'L_knee', 'R_knee'])
    else:
        fixed_frames.append(['torso', 'LF', 'RF'])
    motion_frames_seq.add_motion_frame({
        'RH': final_rh_pos,
        'LH': final_lh_pos
    })
...
def get_five_stage_on_knocker_contact_sequence(robot_name: str,
    ...
    starting_lf_pos = safe_regions_mgr_dict['LF'].iris_list[0].seed_pos
    starting_lh_pos = safe_regions_mgr_dict['LH'].iris_list[0].seed_pos
    starting_rh_pos = safe_regions_mgr_dict['RH'].iris_list[0].seed_pos
    ...
    if B_USE_KNEES:
        ...
        fixed_frames.append(['torso', 'LF', 'RF', 'RH'])
...
def main(args):
    ...
    if kin_plan_path is None:
        ...
        if robot_name == 'valkyrie':
            ...
            if env == 'door':
                if contact_seq == 0:    # step through door
                    ...
                    fixed_frames_seq, motion_frames_seq = get_five_stage_two_hand_contact_sequence(robot_name, p_init, final_pose)
        ...
        if not B_BASELINE:
            ...
            if env == 'door':
                door_to_geom_names = {}
                door_to_geom_names['bottom'] = 'door_frame_0'
                door_to_geom_names['left'] = 'door_frame_1'
                door_to_geom_names['right'] = 'door_frame_2'
    ...
    if B_VISUALIZE_KIN:
        ...
        for t in np.linspace(0, n_contacts * T, N_knots // save_freq):
            if kin_plan_path is None:
                ...
                kin_display.animate_single_collision(plan_to_model_frames['LF'] + '_0', get_frame_des_pos(ik_cfree_planner[1], t))
                kin_display.animate_single_collision(plan_to_model_frames['RF'] + '_0', get_frame_des_pos(ik_cfree_planner[2], t))
                kin_display.animate_single_collision(plan_to_model_frames['L_knee'] + '_0', get_frame_des_pos(ik_cfree_planner[3], t))
                kin_display.animate_single_collision(plan_to_model_frames['R_knee'] + '_0', get_frame_des_pos(ik_cfree_planner[4], t))
    ...
    dyn_solver_stats = robot_dyn_plan.solver_stats
    ...
    if B_SAVE_DYN_DATA:
        ...
        if kin_plan_path is None:
            dyn_data_saver.add('kin_solver_stats', kin_solver_stats)
        dyn_data_saver.add('dyn_solver_stats', dyn_solver_stats)
