...
def get_five_stage_two_hand_contact_sequence(robot_name, starting_pose: dict, final_pose: dict):
    if robot_name == 'g1':
        # G1 settings
    ...
    final_lf_pos    = final_pose['LF']
    final_rf_pos    = final_pose['RF']
    final_torso_pos = final_pose['torso']
    final_rh_pos    = final_pose['RH']
    final_lh_pos    = final_pose['LH']
    ...
    if robot_name == 'g1':
        motion_frames_seq.add_motion_frame({
                                            ...
                                            # 'torso': starting_pose['torso'] + np.array([0.07, -0.07, 0.02])
                                            })
    elif robot_name == 'ergoCub':
        motion_frames_seq.add_motion_frame({
                                            'LH': door_l_inner_location,
                                            # 'torso': starting_pose['torso'] + np.array([0.05, -0.07, 0])
                                            })
    ...
    motion_frames_seq.add_motion_frame({
                        ...
                        # 'L_knee': final_pose['L_knee'] + np.array([-0.05, 0., 0.07])})
    lf_contact_over = PlannerSurfaceContact('LF', np.array([0, 0, 1]))
    motion_frames_seq.add_contact_surfaces([lf_contact_over])
    ...
    fixed_frames.append(['LF', 'RF', 'L_knee', 'R_knee'])   # frames that must not move
    motion_frames_seq.add_motion_frame({
                        # 'LH': starting_pose['LH'] + np.array([0.3, 0., 0.0]),   # <-- G1
                        # 'LH': starting_pose['LH'] + np.array([0.35, 0.1, 0.0]),   # <-- other
    ...
    # ---- Step 5: square up (both hands release to final position)
...
def plan_kin_dyn(robot_name, rob_model, col_model, vis_model,
                 vis_data, col_data,
                 q0, door_pose,
                 plan_to_model_frames, plan_to_model_ids, force_joint_frames, ee_halfspace_params,
                 root_to_torso_offset, standing_pos,
                 safe_regions_mgr_dict,
                 rob_data,
                 planner_params,
                 package_dir, robot_urdf_file,
                 aux_frames_path, fixed_frames_seq, motion_frames_seq, contact_seq_planes,
                 p_init, x0):
    # Note: currently, tailored to G1 on the door environment ONLY
    print(f'{20 * "-"} Planning kinematic paths for {robot_name} {20 * "-"}')
    ...
    if B_VISUALIZE:
        ...
        kin_display.add_robot("door", door_model, door_collision_model, door_visual_model, door_pose[:3], door_pose[3:])
    ...
    if B_VISUALIZE:
        ...
        display.add_robot("door", door_model, door_collision_model, door_visual_model, door_pose[:3], door_pose[3:])
        ...
        if B_SAVE_HTML:
            display.save_html(cwd + "/data/ONR/", robot_name + sca_str + "DYN_anim.html")
...
def main(args):
    ...
    if robot_name == 'valkyrie':
        ...
        if env == 'door':
            # step through door
            final_pose = {fr: pos + np.array([step_length, 0., 0.]) for fr, pos in p_init.items()}
            fixed_frames_seq1, motion_frames_seq1 = get_five_stage_two_hand_contact_sequence(robot_name, p_init, final_pose)
            contact_seqs1 = get_contact_seq_from_fixed_frames_seq(fixed_frames_seq1)
            contact_seq_planes1 = get_contact_planes_from_motion_frames_seq(contact_seqs1, motion_frames_seq1)

            # step on knee-knocker RF
            fixed_frames_seq2, motion_frames_seq2 = get_five_stage_on_knocker_contact_sequence(robot_name, safe_regions_mgr_dict)
            contact_seqs2 = get_contact_seq_from_fixed_frames_seq(fixed_frames_seq2)
            contact_seq_planes2 = get_contact_planes_from_motion_frames_seq(contact_seqs2, motion_frames_seq2)

            # step on knee-knocker RF both hands
            fixed_frames_seq3, motion_frames_seq3 = get_on_knocker_balanced_contact_sequence(robot_name, safe_regions_mgr_dict)
            contact_seqs3 = get_contact_seq_from_fixed_frames_seq(fixed_frames_seq3)
            contact_seq_planes3 = get_contact_planes_from_motion_frames_seq(contact_seqs3, motion_frames_seq3)

    kin_args1 = [
        robot_name, rob_model, col_model, vis_model, vis_data, col_data, q0, door_pose,
        plan_to_model_frames, plan_to_model_ids, force_joint_frames, ee_halfspace_params,
        root_to_torso_offset, standing_pos,
        safe_regions_mgr_dict,
        rob_data,
        planner_params,
        package_dir, robot_urdf_file,
        aux_frames_path, fixed_frames_seq1, motion_frames_seq1, contact_seq_planes1,
        p_init, x0
    ]
    kin_args2 = [
        robot_name, rob_model, col_model, vis_model, vis_data, col_data, q0, door_pose,
        ...
        planner_params,
        package_dir, robot_urdf_file,
        aux_frames_path, fixed_frames_seq2, motion_frames_seq2, contact_seq_planes2,
        p_init, x0
    ]
    kin_args3 = [
        robot_name, rob_model, col_model, vis_model, vis_data, col_data, q0, door_pose,
        ...
        planner_params,
        package_dir, robot_urdf_file,
        aux_frames_path, fixed_frames_seq3, motion_frames_seq3, contact_seq_planes3,
        p_init, x0
    ]

    t1 = threading.Thread(target=plan_kin_dyn, args=kin_args1)
    t2 = threading.Thread(target=plan_kin_dyn, args=kin_args2)
    t3 = threading.Thread(target=plan_kin_dyn, args=kin_args3)
    t1.start()
    t2.start()
    t3.start()
    t1.join()
