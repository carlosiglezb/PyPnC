...
def generate_t_pose_joint_dict(joint_id):
    for k, v in joint_id.items():
        if k == 'leftShoulderPitchLink':
            q_t_pose[k] = np.pi/2.
        elif k == 'rightShoulderPitchLink':
            q_t_pose[k] = -np.pi/2.
        else:
            q_t_pose[k] = 0.
...
robot = PinocchioRobotSystem(
                ...
                cwd + "/robot_model/valkyrie", b_fixed_base=True)
...
connected_frames_list.append(
                    {'parent_frame': 'leftKneePitchLink',
                     'child_frame': 'leftFoot',
                     'length': 0.})    # <-- this will be computed and saved later
connected_frames_list.append(
                    {'parent_frame': 'rightKneePitchLink',
                     'child_frame': 'rightFoot',
                     'length': 0.})    # <-- this will be computed and saved later
...
with open(save_loc + robot_name + '_ee_offsets.yaml', 'w') as f:
...
with open(save_loc + robot_name + '_aux_frames.yaml', 'w') as f:
