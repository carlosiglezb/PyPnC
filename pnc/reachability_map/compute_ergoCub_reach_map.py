...
def generate_t_pose_joint_dict(joint_id):
    for k, v in joint_id.items():
        if k == 'l_shoulder_pitch':
            q_t_pose[k] = np.pi/2.
        elif k == 'r_shoulder_pitch':
            q_t_pose[k] = -np.pi/2.
        else:
            q_t_pose[k] = 0.
...
robot = PinocchioRobotSystem(
                ...
                cwd + "/robot_model/ergoCub", b_fixed_base=True)
...
connected_frames_list.append(
                    {'parent_frame': 'l_lower_leg',
                     'child_frame': 'l_ankle_2',
                     'length': 0.})    # <-- this will be computed and saved later
connected_frames_list.append(
                    {'parent_frame': 'r_lower_leg',
                     'child_frame': 'r_ankle_2',
                     'length': 0.})    # <-- this will be computed and saved later
